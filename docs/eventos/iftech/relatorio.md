# IFTECH – Relatório Institucional

## 1. Identificação do Evento

- Nome: IFTECH – Feira de Inovação Tecnológica
- Edição: 2025
- Data: 04 de novembro de 2025
- Local: IFPR Campus Paranavaí

## 2. Objetivo

A IFTECH tem como finalidade promover a apresentação de projetos desenvolvidos pela comunidade acadêmica, integrando ensino, pesquisa, extensão e inovação, além de fortalecer a cultura institucional de empreendedorismo e desenvolvimento tecnológico.

## 3. Dados Quantitativos

- Projetos apresentados: 64
- Estudantes participantes: 148
- Servidores orientadores: 46
- Avaliadores externos: ___
- Público estimado: ___

## 4. Estrutura Organizacional

- Comissão organizadora:
- Apoio institucional:
- Parcerias:

## 5. Processo de Avaliação

Breve descrição do formato adotado, critérios utilizados e metodologia aplicada.

## 6. Principais Resultados

- Projetos classificados
- Indicações para eventos externos
- Destaques institucionais

## 7. Desafios Identificados

- Logísticos
- Operacionais
- Administrativos
- Comunicação

## 8. Boas Práticas Consolidadas

- Estratégias que funcionaram bem
- Processos que devem ser mantidos nas próximas edições

## 9. Recomendações para Próximas Edições

- Ajustes necessários
- Melhorias estruturais
- Sugestões estratégicas

## Encaminhamentos Administrativos

[Orientações sobre relatóro final e prestação de contas](../../arquivos/Relatorio - Orientações sobre Relatório Final e Prestação de Contas da IFTECH 2025.pdf)

[Orientações sobre devolução de recursos](../../arquivos/Relatorio - Devolução de valores não utilizados – Projetos IFTECH 2025 (Modalidade CIEC).pdf)

Este relatório tem caráter institucional e visa subsidiar futuras edições da IFTECH, garantindo continuidade administrativa, transparência e aperfeiçoamento constante do evento.
